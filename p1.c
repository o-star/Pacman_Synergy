...
void multi_gameversion()
{
    ...
    client_data.sock_fd=sock_fd;
    pthread_create(&pid,NULL,(void *)read_thread,(void *)&client_data);


    while(message==-1){
        attron(A_BLINK);
        mvaddstr(TOWERBOTTOM / 2 - 3, LEFTEDGE + 25, "Wait Your User");
        attroff(A_BLINK);
        refresh();
        // 대전상대 기다리기
    }
    ...
    while (1) {
        ...
        if(message==1){ // message가 1이면 자신의 턴
            now_player=1;
        }
        else if(message==2){ // message가 2라면 다른사람의 턴
            now_player=0;
        }
...
void* read_thread(void* data){
    ...
    struct client* user = data;
    int check=0;

    while(1){
        check=read(user->sock_fd,&message,sizeof(message));
